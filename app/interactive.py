...
# Carga de todos los datos en caché
@st.cache_data
def load_all_data():
    """Carga todos los datos una sola vez"""
    # Cargar vinos disponibles
    ...
    # Cargar datos de usuarios (para modelo)
    ...
    return wine_df, users_data, meals_df, grapes_list, region_list, pairings_list, taste_columns

wine_df, users_data, meals_df, grapes_list, region_list, pairings_list, taste_columns = load_all_data()
...
def init_session_state():
    ...
    if 'price_range' not in st.session_state:
        st.session_state.price_range = (0, 51)
...
def reset_session():
    ...
    st.session_state.page = 'home'
    st.session_state.price_range = (1, 51)
    st.session_state.taste_preferences = {}
    st.session_state.selected_grapes = []
    st.session_state.current_wine_page = 0
...
def show_home_page():
    ...
    with col1:
        if st.button("🍽️🍷 Encontrá el Vino Ideal para tu Comida", 
    ...
    with col2:
        if st.button("🔎🍷 Descubrí Nuevos Vinos", 
...
def show_meal_selection():
    ...
    st.markdown('<div class="subsection-header">🍽️ Comida a Acompañar</div>', unsafe_allow_html=True)
    ...
    with st.container(border=True):
        ...
        st.markdown("#### Seleccioná una comida para encontrar su vino ideal")
        st.markdown("---")
        ...
        if selected_meal:
            ...
            with food_col2:
                st.markdown("#### 🔎 Características de tu Comida Elegida")
...
def show_price_selection():
    st.markdown('<div class="subsection-header">💰 Precio</div>', unsafe_allow_html=True)
    ...
    with st.container(border=True):
        st.markdown("### 💵 ¿Cuál es tu presupuesto?")
        st.markdown("#### Seleccioná un precio mínimo y uno máximo")
        st.markdown("---")
...
def show_taste_selection():
    st.markdown('<div class="subsection-header">👅 Perfil de Sabor</div>', unsafe_allow_html=True)
    ...
    with st.container(border=True):
        st.markdown("### 👃👄 ¿Qué tipos de vinos te gustan?")
        st.markdown("#### Definí la intensidad deseada para cada característica del vino")
        st.markdown("---")
        ...
        taste_characteristics = {
            ...
            "tannins": {
                "emoji": "🌿",
                "name": "Taninos presentes en el vino",
                "short_name": "Taninos",
        ...
        for i, (key, info) in enumerate(taste_characteristics.items()):
            with summary_cols[i]:
                ...
                <div style="text-align: center; padding: 10px; background-color: #f8f9fa; border-radius: 8px;">
                    ...
                    <div style="font-weight: bold;">{info['short_name']}</div>
...
def show_grape_selection():
    st.markdown('<div class="subsection-header">🍇 Uvas</div>', unsafe_allow_html=True)
    
    with st.container(border=True):
        # Navegación
        nav_col1, nav_col2, nav_col3, nav_col4 = st.columns([1.1, 1, 1, 1])
        
        with nav_col1:
            if st.button("← Cambiar Perfil de Sabor"):
                st.session_state.page = 'taste_selection'
                st.rerun()
        ...
        taste_prefs_keys = list(taste_prefs.keys())
        taste_prefs_values = list(taste_prefs.values())

        with nav_col3:
            st.markdown(
                f"""
                - 🏋️ **{taste_prefs_keys[0].title()}**: {taste_prefs_values[0]}
                - 🌿 **{taste_prefs_keys[1].title()}**: {taste_prefs_values[1]}
                """
            )
        ...
        with nav_col4:
            st.markdown(
                f"""
                - 🍯 **{taste_prefs_keys[2].title()}**: {taste_prefs_values[2]}
                - 🍋 **{taste_prefs_keys[3].title()}**: {taste_prefs_values[3]}
                """
            )


    with st.container(border=True):
        st.markdown("### 🍇 ¿Qué Uvas te Gustan?")
        st.markdown("#### Elegí las uvas que te gustaría probar")
        st.markdown("---")
...
def show_wine_results(trained_model):
            ...
            with st.spinner("🔍 Buscando los vinos perfectos para ti..."):
                ...
                if st.session_state.selected_grapes:
                    ...
                    if "otras uvas" in selected_grapes:
                        ...
                        other_grapes = [g.lower() for g in grapes_list if g.lower() not in grapes_for_selection]
    ...
    with st.container(border=True):
        ...
        filtered_wines = filtered_wines.sort_values(by=sort_options.get(sorter), ascending=is_ascending)
        ...
        option_col1, option_col2, option_col3 = st.columns([0.3, 0.5, 0.2])
